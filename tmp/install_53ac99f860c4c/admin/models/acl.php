...
class KomentoModelAcl extends KomentoParentModel
{
	...
	public function updateUserGroups( $component )
	{
		...
		// Reformat the array
		$defaultUserGroupIDs = array();
		foreach( $result as $row )
		{
			$defaultUserGroupIDs[$row->cid] = $row->rules;
		}

		foreach( $userGroupIDs as $userGroupID )
		{
			if( !in_array( $userGroupID, $current ) && isset( $defaultUserGroupIDs[$userGroupID] ) )
			{
				$table = Komento::getTable( 'acl' );
				$table->cid = $userGroupID;
				$table->component = $component;
				$table->type = 'usergroup';
				$table->rules = $defaultUserGroupIDs[$userGroupID];
	...
	public function save( $data )
	{
		...
		$defaultset = $this->getEmptySet( true );
